...
系统定位是 **passive runtime intelligence**，而不是“只读查看器”：

- 可部署在开发者工作站，也可作为经过认证的自托管远程服务运行；
- 默认采用非干预、只读采集，不修改被观察的 Agent、任务决策或源码工作区；
- 可以写入自己的证据数据库、在线运行，并由用户配置导入与导出；
- 运行证据保留在操作者控制的部署边界内，除非显式启用外部数据流。

因此，“只读”修饰的是采集和干预边界，不是整个产品的读写能力。部署位置与
可观测互操作也是两个独立维度：本机或自托管远程部署都可以不连接外部平台，
Trace 导入与 OTLP/HTTP 导出也都必须由操作者显式配置。
...
开发者工作站与自托管远程服务是部署位置；Trace 导入、OTLP/HTTP 导出是数据
互操作方式。两者相互独立，都不是产品定位和市场边界。
...
- Collector、证据存储和 UI 运行在操作者管理的远程环境；
- 来源端通过显式配置的安全入口发送最小化运行证据；
- 查看与采集使用独立凭据，并要求 TLS 或 HTTPS 反向代理边界；
- 不依赖厂商托管账号，也不等同于多租户 SaaS。
...
- 支持版本化的 Trace 导入；
- 支持显式启用的 OTLP/HTTP 标准化证据导出；
- 导出失败不能阻断 Agent，也不能改变本地或远程部署的核心诊断能力；
- 连接外部可观测平台不是本地或远程部署的前提。
...
- 默认非干预、只读采集；支持工作站与自托管远程部署。
- 证据保留在操作者控制的部署边界内；导入与导出必须显式配置。
- 不代理模型请求。
- 默认不阻断 Agent 行为。
