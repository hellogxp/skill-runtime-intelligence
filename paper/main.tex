...
passive runtime-intelligence system deployable on a developer workstation or as
a self-hosted remote service. It defaults to non-intervening collection and
retains evidence within the operator-controlled deployment boundary; optional
trace import and OTLP/HTTP export provide interoperability with existing
observability platforms. It reconstructs supported Skill-lifecycle stages
across heterogeneous harnesses while preserving unsupported stages as unknown;
its \panorama separates immutable events, deterministic relations, inferred
diagnoses, and controlled outcomes with four evidence grades.
...
correlate to one source session. Yet adapters expose three distinct semantics:
...
implementation supports a loopback-local service or an authenticated
self-hosted remote service with separate read-only viewer and write-only
Collector credentials; trace import and OTLP/HTTP export remain independent
options. The architecture complements rather than replaces span-centric
observability: OpenTelemetry's generative-AI conventions describe agent
invocation and tool execution spans \cite{otel-genai}, whereas \system adds a
domain layer centered on a versioned Skill definition and one occurrence of
that definition.
...
\cite{swebench}, while AgentBoard measures incremental task progress beyond
final success \cite{agentboard}. These efforts evaluate agent capability; our focus
...
The product defaults to local, non-intervening collection. Its authenticated
self-hosted mode keeps viewer and Collector roles separate and retains evidence
inside the operator-controlled deployment boundary; observability export is
...
the audited command discovers 276 tests with three environment skips, and the
selected suite passes 13/13 gates. A versioned, de-identified,
\href{https://github.com/hellogxp/skill-runtime-intelligence/releases/tag/profes-2026-artifact-v1}{public artifact}
