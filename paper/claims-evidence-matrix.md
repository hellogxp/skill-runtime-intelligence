...
| C3 | Under the controlled fault contract, Codex reconstructed 0/42 SkillRuns despite 42/42 target-`SKILL.md` path signatures and exact nonce-bound responses; OpenCode emitted failure-like events in 0/24 operational failure cells; Qoder emitted them in 24/24 operational failure cells and 6/6 clean cells, with 6/24 exact injected boundaries | Agent and reconstruction confirmatory reports | Path signatures establish hidden activation semantics; Qoder detected or was nonce-attributed to every injected failure; these are production incident rates |
...
| C12 | On six controlled rule-external anomaly/clean pairs, Qwen precision/recall/F1 was 1.00/.67/.800 and DeepSeek was .86/1.00/.923; support-valid relations occurred in 10/12 and 11/12 cases with three disagreements | novel-pattern model and cross-model reports | These controlled error directions characterize either model generally, estimate unknown-fault accuracy, or estimate natural incident prevalence |
| C13 | The implementation can run as an authenticated self-hosted remote service independently of observability import/export, with direct TLS or a loopback HTTPS-proxy backend, separate read-only viewer/write-only Collector credentials, and fail-open queue relay | `tests/test_remote_deployment.py`, the clean public `profes-2026-artifact-v2` snapshot (279 passed, 10 environment-dependent skips, four subtests), the CLI cross-process check, and a one-event Mac-to-PAI-DSW SSH-tunnel smoke | Production availability, Internet-scale reliability, direct-HTTPS cross-host behavior, multi-tenant isolation, or a measured remote-deployment overhead bound |
...
- Treat `target_skill_loaded` and `target_probe_executed` as format-dependent
  path-signature detectors, not activation or execution oracle labels.
