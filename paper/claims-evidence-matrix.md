...
| C13 | The implementation can run as an authenticated self-hosted remote service independently of observability import/export, with direct TLS or a loopback HTTPS-proxy backend, separate read-only viewer/write-only Collector credentials, and fail-open queue relay | `tests/test_remote_deployment.py`, the complete 285-test suite, and the CLI cross-process service/relay check | Production availability, Internet-scale reliability, multi-tenant isolation, or a measured remote-deployment overhead bound |
