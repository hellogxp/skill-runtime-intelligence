(() => {
  ...
  const zh = {
    ...
    "Detected · hook consent not granted": "已检测 · 尚未同意 Hook",
  ...
  const zhPatterns = [
    ...
    [/^(\d+) fallback$/, "$1 个回退来源"],
    [/^(\d+) imports?$/, "$1 个导入来源"],
    [/^(\d+) live hooks?$/, "$1 个实时 Hook"],
    [/^(\d+) hooks? pending$/, "$1 个 Hook 待验证"],
    ...
    [/^(\d+) official events · live evidence verified · fail-open$/, "$1 个官方事件 · 实时证据已验证 · fail-open"],
    [/^(\d+) events configured · awaiting Agent trust or a new run$/, "$1 个事件已配置 · 等待 Agent 信任或新运行"],
  ...
  const patternRules = [
    ...
    [/^(\d+) live hooks?$/, "live_hook_count", ["count"]],
    [/^(\d+) hooks? pending$/, "pending_hook_count", ["count"]],
    ...
    [/^(\d+) official events · live evidence verified · fail-open$/, "official_events", ["count"]],
    [/^(\d+) events configured · awaiting Agent trust or a new run$/, "configured_events", ["count"]],
