...
当前 Skill 定义中提取保守、可检查的行为约束，与真实运行活动进行匹配，再将 Agent
官方事件、导入 Trace、明确标识的 session fallback 和可观察工作区结果重建为证据
分级的 Skill Run Panorama。SRI 可部署在开发者工作站，也可作为经过认证的自托管
远程服务运行。系统默认采用非干预、只读采集：不代理模型请求、不接管 Agent loop、
...
推断因果有效性。Collector、证据存储和 UI 既可共同运行在开发者工作站，也可运行
在经过认证的自托管部署中；可观测数据的导入与导出不会决定部署位置。
...
- 默认由操作者控制，可部署在开发者工作站或自托管远程环境。
- 默认非干预、只读采集；不接管 Agent loop、不阻断 Agent 行为，也不修改被观察的
  源码工作区。
- 不代理模型请求，也不强制依赖云服务。
- 默认产品不阻断、不审批、不执行策略。
- Trace 导入与 OTLP/HTTP 导出均需显式启用，并与部署位置相互独立。
- 所有结论都带来源与证据等级。
...
部署位置与可观测互操作相互独立。SRI 可运行在本机，也可作为经过认证的自托管
远程服务运行；选择远程部署并不要求连接外部可观测平台。
